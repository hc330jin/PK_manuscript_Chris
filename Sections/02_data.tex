...
\end{equation}
If multiple transactions are equally close, the larger transaction is selected, followed by the larger trade identifier as a deterministic tie-breaker.For diagonostic purposes, the constructed dataset also records the quantity-weighted average price and median transaction price within each window. These alternative price measures are not used in the baseline calibration, which relies exclusively on the transaction closest to 08:00 UTC. This panel is not an order-book snapshot: contracts without a qualifying transaction are not imputed.

Table~\ref{tab:closing_window_comparison_15min} compares 08:00-centered bandwidths from one to thirty minutes and reports the full UTC day as an unfiltered benchmark. Wider windows increase the number of available date--contract observations, but also move the selected trade farther from the target valuation time. With no bandwidth restriction, the data contain 186,237 date--instrument observations, but these trades may occur anywhere within the same UTC day and therefore do not form a contemporaneous closing cross-section. The 15-minute half-window provides a practical compromise: it increases annual coverage from 27,018 observations under the five-minute window to 46,788 observations, and it captures a meaningfully broader set of longer-maturity contracts while retaining a tight 30-minute calendar window around the settlement time. We therefore use the 15-minute filtered panel as the experimental option dataset.
...
\toprule
Half-window & Full window & \multicolumn{2}{c}{Daily trades} & \multicolumn{2}{c}{Daily instruments} & Annual & Reserved\\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
$h$ & width & Mean & Median & Mean & Median & observations & percentage\\
\midrule
$\pm 1$ min & 2 min & 52.76 & 45 & 22.27 & 21 & 8,151 & 4.38\%\\
\textcolor{red}{$\pm 2.5$ min} & \textcolor{red}{5 min} & \textcolor{red}{190.65} & \textcolor{red}{173.5} & \textcolor{red}{49.02} & \textcolor{red}{44} & \textcolor{red}{17,940} & \textcolor{red}{9.63\%}\\
$\pm 5$ min & 10 min & 411.99 & 366 & 73.82 & 66 & 27,018 & 14.51\%\\
$\pm 10$ min & 20 min & 607.66 & 557 & 105.27 & 96 & 38,528 & 20.69\%\\
$\pm 15$ min & 30 min & 756.87 & 698 & 127.84 & 118 & 46,788 & 25.12\%\\
$\pm 20$ min & 40 min & 895.96 & 819 & 146.49 & 137 & 53,616 & 28.79\%\\
$\pm 30$ min & 60 min & 1151.52 & 1034 & 176.33 & 166 & 64,537 & 34.65\%\\
None & 24 h & 13,595.34 & 12,435.5 & 508.84 & 501.5 & 186,237 & 100\%\\
\bottomrule
\end{tabular}
\caption{\textbf{Comparison of daily closing-price windows.} For every UTC date in 2024, the table summarizes eligible regular outright BTC inverse-option transactions. The symmetric windows are centered on 08:00 UTC; the final row uses the entire UTC calendar day as an unfiltered benchmark. ``Daily instruments'' is the number of distinct contracts with at least one eligible trade, and ``Annual observations'' is the number of unique date--instrument pairs.}
...
\textcolor{red}{For the descriptive figures and tables in this subsection, the 08:00 BTC index-price proxy is used to summarize the strike distribution and moneyness. In the baseline calibration, however, $S_0$ is the contemporaneous Deribit index price recorded with each selected option transaction. Contracts retained on the same date can therefore have slightly different values of $S_0$ because their transaction timestamps need not coincide exactly.}
...
\textcolor{red}{The descriptive maturity tables below report accurate time to maturity measured from the selected transaction timestamp to expiration. The baseline SVCJ calibration instead measures $T$ from the common 08:00 UTC valuation time to the 08:00 UTC expiration time and expresses it as an integer number of calendar days, consistent with the model's daily simulation step. Contracts expiring at the valuation time ($T=0$) are excluded. Hence, the transaction-time-adjusted maturity field is retained for sample description but is not used by the baseline calibration. A selected trade may occur up to 15 minutes before or after 08:00 UTC; this small discrepancy is treated as a measurement approximation around the common daily valuation time.}
...
\textcolor{red}{Transaction-quantity weighting is used only for these descriptive summaries. The baseline SVCJ calibration is not weighted by transaction quantity or by the number of trades in the window: after selecting one closest-to-08:00 transaction for each date--instrument pair, every retained option contract contributes one relative pricing residual to the least-squares objective.}
