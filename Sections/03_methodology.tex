...
{\color{red}
We generate 20 independent master paths of 1,000 daily returns. Within each
replication, estimation uses the nested prefixes
\begin{equation}
\mathcal{Y}_{10}\subset\mathcal{Y}_{100}
\subset\mathcal{Y}_{1000},
\end{equation}
...
dispersed parameter vectors. Each chain contains 50,000 MCMC sweeps, of
which the first 25,000 are discarded. The sampler combines
...
effective sample size (ESS), and tail ESS. We report two diagnostic counts.
The strict count requires every reported parameter and the stored conditional
log-likelihood statistic to have $\widehat R\leq1.01$, bulk ESS $\geq400$,
tail ESS $\geq400$, and no nonfinite draws. For comparison with the reference
sampler's diagnostic convention, the second count requires only
$\widehat R<1.05$ for these quantities, without an ESS requirement. An
$\widehat R$-only pass does not by itself establish reliable posterior
estimation. Estimates from failed groups remain in the table rather than
...
& Estimate & APE (\%)\\
\midrule
$V_0$      & 0.000700 & 0.001622 & 131.7 & 0.001643 & 135.7 & 0.001507 & 120.7\\
$\mu$      & 0.002000 & $-0.001046$ & 514.6 & 0.001217 & 161.2 & 0.002039 & 53.0\\
$\mu_y$    & $-0.010000$ & 0.002843 & 141.8 & $-0.000228$ & 487.5 & $-0.007910$ & 188.0\\
$\sigma_y$ & 0.110000 & 0.121619 & 10.6 & 0.113399 & 26.4 & 0.114069 & 11.5\\
$\lambda$  & 0.050000 & 0.052060 & 5.3 & 0.051021 & 23.7 & 0.047360 & 22.9\\
$\alpha$   & 0.000035 & 0.002400 & 6756.3 & 0.000745 & 2029.0 & 0.000088 & 151.3\\
$\beta$    & 0.950000 & 0.102179 & 89.2 & 0.413204 & 56.5 & 0.903798 & 4.9\\
$\rho$     & $-0.200000$ & 0.005299 & 102.6 & $-0.068656$ & 143.8 & $-0.186832$ & 50.6\\
$\sigma_v$ & 0.005000 & 0.012583 & 151.7 & 0.009269 & 85.4 & 0.006624 & 32.5\\
$\rho_j$   & $-0.500000$ & 0.020168 & 104.0 & 0.025791 & 188.3 & $-0.106264$ & 358.9\\
$\mu_v$    & 0.000600 & 0.022903 & 3717.1 & 0.004963 & 727.2 & 0.001120 & 86.6\\
$\kappa$   & 0.050000 & 0.897821 & 1695.6 & 0.586796 & 1073.6 & 0.096202 & 94.0\\
$\theta$   & 0.000700 & 0.006219 & 788.4 & 0.001190 & 77.6 & 0.000920 & 35.7\\
\midrule
All diagnostics pass & & \multicolumn{2}{c}{20/20}
& \multicolumn{2}{c}{0/20} & \multicolumn{2}{c}{0/20}\\
$\widehat R<1.05$ only & & \multicolumn{2}{c}{20/20}
& \multicolumn{2}{c}{12/20} & \multicolumn{2}{c}{13/20}\\
\bottomrule
\end{tabular}%
}
...
four chains with 25,000 retained draws per chain. The penultimate row reports
groups satisfying $\widehat R\leq1.01$ and bulk/tail ESS $\geq400$ for all
reported parameters and the stored conditional log-likelihood; the final row
uses only $\widehat R<1.05$ for those quantities. APE is computed within each
replication before averaging and is therefore not generally equal to the
percentage error of the reported mean.}
\label{tab:p_parameter_recovery_simulation}
\end{table}

{\color{red}
Table~\ref{tab:p_parameter_recovery_simulation} shows substantial improvement
in the recovery of the mean-reversion parameters as the return sample grows.
Mean APE for $\kappa$ falls from 1,695.6\% at $n=10$ to 1,073.6\% at $n=100$
and 94.0\% at $n=1000$; the corresponding values for $\theta$ are 788.4\%,
77.6\%, and 35.7\%. At $n=1000$, however, the mean $\kappa$ estimate is
0.0962 against a true value of 0.05, and the empirical coverage of the 90\%
credible intervals is only 60\% for both $\kappa$ and $\theta$. The co-jump
loading $\rho_j$ remains weakly recovered, with mean APE increasing to
358.9\% at $n=1000$.
}

{\color{red}
Diagnostic conclusions depend on the stated criterion. All 20 groups at
$n=10$ pass the strict $\widehat R$--ESS criterion, but none do so at $n=100$
or $n=1000$. Under the reference-aligned $\widehat R<1.05$ criterion alone,
12/20 groups pass at $n=100$ and 13/20 at $n=1000$. The high pass rate for the
shortest sample does not imply accurate recovery: ten returns provide little
information about the full SVCJ parameter vector. Conversely, an
$\widehat R$-only pass does not resolve low ESS in the longer samples. Thus the
APE trend supports improvement with sample length, while the remaining mixing
and coverage limitations preclude claiming reliable joint recovery at 1,000
returns.
}
...
that even 1,000 observations do not reliably recover the full parameter vector.
...
{\color{red}
We therefore retain 1,000 returns as a pragmatic baseline rolling window. For a market
...
$\lambda^P=0.05$. The simulation shows improved recovery at this length, but
does not validate full-vector inference or establish that this window is
optimal. An expanding window and a 2,000-return window therefore
